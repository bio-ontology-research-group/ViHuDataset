...
@ck.option('--num_walks', '-nw', default=100, type=int)
@ck.option('--walk_length', '-wl', default=30, type=int)
...
def main(embedding_dimension, epochs, window_size, num_walks,
    ...
    wandb_logger = wandb.init(entity='ferzcam', project='hpi_analysis', group='dl2vec', name=wandb_description)
    ...
    model_filepath = os.path.join(out_dir, f"dl2vec_{embedding_dimension}_{epochs}_{window_size}_{num_walks}_{walk_length}.model")
    corpus_filepath = os.path.join(out_dir, f"corpus_dl2vec_{embedding_dimension}_{epochs}_{window_size}_{num_walks}_{walk_length}.txt")

    model = RandomWalkPlusW2VModel(dataset, model_filepath=model_filepath)
    model.set_projector(DL2VecProjector(bidirectional_taxonomy = False))
    model.set_walker(DeepWalk(num_walks, walk_length, workers=10, outfile=corpus_filepath))
