...
for i in range(len(results)):
    ...
    assert len(scores) == len(genes_ids)
    ...
    # print(f"Virus: {virus}, position: {position}, updated_position: {updated_position}, rank: {rank}")
    ...
    if virus not in micro_mr:
        micro_mr[virus] = 0
        micro_mrr[virus] = 0
        micro_hits_k[virus] = {1: 0, 3: 0, 10: 0, 100: 0}
        micro_ranks[virus] = dict()
        num_results_per_virus[virus] = 0

    num_results_per_virus[virus] += 1
        
    micro_mr[virus] += rank
    micro_mrr[virus] += 1 / rank
    for k in micro_hits_k[virus]:
        if rank <= int(k):
            micro_hits_k[virus][k] += 1

    if rank not in micro_ranks[virus]:
        micro_ranks[virus][rank] = 0
    micro_ranks[virus][rank] += 1
    
    

print(num_results_per_virus)
macro_metrics = dict()
...
micro_mr = {k: v / num_results_per_virus[k] for k, v in micro_mr.items()}
micro_mrr = {k: v / num_results_per_virus[k] for k, v in micro_mrr.items()}
micro_auc = {k: compute_rank_roc(micro_ranks[k], len(scores)) for k in micro_ranks}
for k in micro_hits_k:
    micro_hits_k[k] = {k2: v / num_results_per_virus[k] for k2, v in micro_hits_k[k].items()}
