...
class HPIEvaluator(Evaluator):
    ...
    def get_filtering_labels(self, num_heads, num_tails, class_id_to_head_id, class_id_to_tail_id, **kwargs):
        ...
        print(f"Filtering tuples: {filtering_tuples.shape}")
        print(f"Filtering labels: {filtering_labels.shape}")
...
class HPIEvaluatorNN(HPIEvaluator):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def create_tuples(self, ontology):
        human_genes, virus = self.dataset.evaluation_classes

        print(f"HPI Evaluator NN: {len(human_genes)} human genes and {len(virus)} virus proteins")
        
        human_genes_to_id = {c: i for i, c in enumerate(human_genes.as_str)}
        virus_to_id = {c: i for i, c in enumerate(virus.as_str)}
        ...
        for e in edges:
            head = human_genes_to_id[e.src]
            relation = 0
            tail = virus_to_id[e.dst]
            edges_indexed.append((head, relation, tail))
        ...
        with th.no_grad():
            for batch, in dataloader:
                ...
                for i, tail in enumerate(aux_tails):
                    head = aux_heads[i]
                    perm = th.randperm(num_heads)
                    evaluation_heads = th.arange(len(self.evaluation_heads), device=head.device)
                    evaluation_tails = th.arange(len(self.evaluation_tails), device=tail.device)
                    head = th.where(evaluation_heads[perm] == head)[0].item()
                    tail = th.where(evaluation_tails == tail)[0].item()
